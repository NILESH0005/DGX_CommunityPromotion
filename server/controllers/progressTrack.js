...
export const getUserFileIDs = async (req, res) => {
    ...
    try {
        connectToDatabase(async (err, conn) => {
            ...
            try {
                const userQuery = `
                    SELECT UserID FROM Community_User 
                ...
                res.status(200).json({ success, data: fileIds, message: successMessage });

            } catch (queryErr) {
                logError(queryErr);
                closeConnection();
                res.status(500).json({ success: false, message: "Query execution failed", data: queryErr });
            }
        });
    } catch (error) {
        logError(error);
        res.status(500).json({ success: false, data: {}, message: "Unexpected error occurred" });
    }
};
