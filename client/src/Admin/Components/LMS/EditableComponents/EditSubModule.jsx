    ...
    try {
      const simplifiedSubmodules = orderedSubmodules.map(
        (submodule, index) => ({
          ...
          ModuleID: submodule.ModuleID, // <-- add this!
    ...
    if (submodule.SubModuleImage?.data) {
      if (Array.isArray(submodule.SubModuleImage.data)) {
        ...
        setImagePreview(
          `data:${
            submodule.SubModuleImage.contentType || "image/jpeg"
          };base64,${submodule.SubModuleImage.data}`
        );
      }
    } else {
      ...
      <div className="space-y-6 p-6">
        <div className="border-b border-gray-200 pb-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage all submodules</p>
          </div>
          <button
                      ...
                      <>
                        {submodule.SubModuleImage?.data ? (
                          typeof submodule.SubModuleImage.data === "string" ? (
                            <img
                              src={`data:${
                                submodule.SubModuleImage.contentType ||
                                "image/jpeg"
                              };base64,${submodule.SubModuleImage.data}`}
