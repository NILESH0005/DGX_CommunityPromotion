...
const EditModule = ({
  ...
  const reloadModuleData = async () => {
    ...
    try {
      const response = await fetchData(
        `lmsEdit/getModule/${module.ModuleID}`, // Changed from initialModule to module
        "GET",
        null,
        { "auth-token": userToken }
      );
      if (response.success) {
        setEditedModule(response.data);
        setImagePreview(
          response.data.ModuleImage?.data
            ? `data:${
                response.data.ModuleImage.contentType || "image/jpeg"
              };base64,${response.data.ModuleImage.data}`
            : null
        );
      }
    } catch (error) {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      // Store the actual content type
      const contentType = file.type; // This will be "image/png", "image/jpeg", etc.
      ...
      // Store both the data and content type
      setNewImageFile({
        data: base64String,
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("leeetss go payload", payload);
    ...
    // Only log if ModuleImage exists
    if (payload.ModuleImage) {
      console.log("ModuleImage data:", payload.ModuleImage.data);
    }
    ...
    try {
      ...
      if (response?.success) {
        await reloadModuleData(); // Refresh data after update
        const updatedModule = {
          ...response.data,
          ModuleName: response.data.ModuleName,
          ModuleDescription: response.data.ModuleDescription,
          ModuleImage: response.data.ModuleImage
            ? {
                data: response.data.ModuleImage.data,
                contentType:
                  response.data.ModuleImage.contentType || "image/jpeg",
              }
            : null,
        ...
        setImagePreview(
          updatedModule.ModuleImage?.data
            ? `data:${updatedModule.ModuleImage.contentType};base64,${updatedModule.ModuleImage.data}`
            : null
        );
        ...
        Swal.fire({
          ...
          if (props.onUpdateSuccess) {
            props.onUpdateSuccess(); // Trigger parent to reload
          }
        });
      } else {
      ...
      <div className="h-48 bg-gradient-to-r from-white-500 to-white-700 overflow-hidden relative group">
        {isImageEditing ? (
          <div className="h-full flex flex-col items-center justify-center p-4 bg-black bg-opacity-70">
            {imagePreview ? (
              ...
              typeof editedModule.ModuleImage === "string" ? (
                ...
                <ByteArrayImage
                  ...
                  contentType={editedModule.ModuleImage.contentType}
                  className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"
                />
              ) : null
            ) : (
              <div className="h-full flex items-center justify-center bg-gradient-to-br from-white-600 to-white-800">
                {isEditing ? (
                  <div className="text-center p-4">
                    <p className="text-black-200 mb-3 text-sm">
                      No Image Available
                    </p>
                    <button
            ...
            {isCompressing && (
              <p className="text-xs text-gray-300 mt-2 animate-pulse">
                Compressing image...
              </p>
            )}
          </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setImagePreview(
                      editedModule.ModuleImage
                        ? `data:image/jpeg;base64,${editedModule.ModuleImage.data}`
                        : null
                    );
                  }}
